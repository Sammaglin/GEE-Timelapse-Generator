...
def get_composite_image(aoi, start_date, end_date, image_type):
    ...
    if image_type == "True Color":
        collection = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2") \
            .filterBounds(aoi).filterDate(start_date, end_date).map(maskL8sr)
...
def create_timelapse(frames, output_path, format):
    ...
    if format == "GIF":
        ...
        imageio.mimsave(output_path, images, fps=2, macro_block_size=None)
    return output_path
...
def index():
    ...
    if request.method == 'GET':
        return render_template_string(HTML_TEMPLATE, folium_map=m._repr_html_(), message=message, download_link=download_link)
